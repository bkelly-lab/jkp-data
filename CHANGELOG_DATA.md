...
## 10-07-2026
__Changes__:
- Applied the Bessembinder, Chen, Choi, and Wei (2023) data corrections to the Compustat daily security files (North America and Global). Section 6 detects and rescales decimal-shift price errors (single- and multi-period spike-and-reversal); Section 8 drops observations that cannot be reliably repaired (filters 8a–8h: low trading volume, zero AJEXDI, sub-threshold price or market equity, long data gaps, implausible shares-outstanding and market-equity jumps, return/market-equity mismatches, and initial-observation ratio errors). The multi-period search is adapted for daily data (symmetric windows over a priority set of 1, 2, 3, 5, 10, and 21 trading days) ([#XXX](https://github.com/bkelly-lab/jkp-data/pull/XXX))
- Recovered returns for never-dividend securities by substituting `trfd = 1` in `gen_comp_dsf` when a security never pays a dividend, so its price return equals its total return instead of being dropped as a missing total-return factor ([#XXX](https://github.com/bkelly-lab/jkp-data/pull/XXX))
- Added a post-correction return screen: Compustat returns above 1000% (`ret` > 10) that survive the Bessembinder correction are set to null in `gen_returns_df` as residual, unrepairable data errors ([#XXX](https://github.com/bkelly-lab/jkp-data/pull/XXX))
- The impact concentrates outside the United States; US factor series are largely unchanged because US observations are sourced primarily from CRSP rather than Compustat ([#XXX](https://github.com/bkelly-lab/jkp-data/pull/XXX))
