...
## 02-09-2026
__Changes__:
- Fixed double-counting of CRSP CIZ delisting returns. Under the CIZ flat-file format, `MthRet`/`DlyRet` may already include the delisting payoff depending on `MthDelFlg`/`DlyDelFlg`. The pipeline now propagates `del_flag` and compounds `DelRet` from `crsp.stkdelists` into `MthRet` unless `del_flag` is `"A"` or `"P"` (payoff already embedded). Flags `"M"` (missing terminal payoff), `"G"` (gap delisting, >10 trading days), `"N"` (no delisting this period, typically a no-op because `delret` is null), and `"V"` (voluntary) all trigger compounding; missing `delret` is imputed at −0.30 for performance-related delisting reason codes when the flag is not `"A"` or `"P"`. Daily returns are never compounded with `DelRet`; `DlyDelFlg` Y/N already reflects or excludes the payoff. See Xia (2026, SSRN 7243220) for background.
