...
- Characteristics data set — new characteristic `eqnpo_60m` (60-month net equity payout, `log(RI_t/RI_t-60) - log(ME_t/ME_t-60)`), computed alongside the existing `eqnpo_1m/3m/6m/12m` horizons and included in `world_data.parquet` and the per-country files. No factor portfolio is constructed for it. This brings the characteristic count to 407.
...
- DHS ROW IR now comes from the new `eqnpo_60m` JKP characteristic it duplicates (`IR = -eqnpo_60m` at the June formation; verified value-identical where both defined, corr 1.000). JKP's endpoint-only guards replace the previous 60-of-60 contiguous-month window gate, a strict coverage superset: ROW June IR observations rise 18.7% (612,794 to 727,099), `ir_dhs` availability in the characteristics files rises 11%, and monthly `fin_dhs` nets +76 country-months (+167/-91; the losses are small-market months where re-ranked buckets fall below the minimum stock count). ROW FIN values shift from the broader eligible pool (changed-month correlation 0.986; US and PEAD unaffected).
- DHS ROW PEAD announcements now include semiannual (SAN) IBES records alongside quarterly (QTR). Semiannual-reporting markets carry their interim announcements under SAN (IBES reclassified UK interims from QTR to SAN in 2008), so their PEAD pools fell below the minimum-stocks breakpoint gate. Restoring them extends `pead_dhs` in those markets (GBR resumes after 2016-06 and now runs 2003-12 to 2025-12, 85 to 255 country-months; AUS gains coverage outright; +824 monthly country-months overall) and raises `abr_dhs` availability 6.5%. Previously covered months re-rank over the larger pools (changed-month correlation 0.91); US PEAD (fundq RDQ path) and FIN are byte-identical.
- DHS ROW PEAD announcement bridge widened twice more: the IBES ticker link now reads both Compustat security masters (`comp_g_security` + `comp_security`; ex-US firms filed under NA coverage previously never bridged — Canada gains `pead_dhs` outright, 438 country-months 1989-06 to 2025-12), and the EPS-only measure filter is dropped (announcement dates are measure-agnostic; FFO/NAV/other-primary reporters now contribute — JPN +88, ZAF +89 country-months). The staleness gate is periodicity-aware (9 months for semiannual-sourced announcements vs 6 for quarterly), recovering the tail months of each semiannual cycle. Net +863 monthly `pead_dhs` country-months; `abr_dhs` availability +14%; US PEAD and FIN byte-identical.
- FF ROW June universe: the Dec(t-1) market-equity join is now left instead of inner (US parity) — June stocks without a prior-December ME (new listings, thin Decembers) stay in the OP/INV sorts with null BEME. HML/SMB(FF3)/UMD are unchanged (B/M sorting still requires Dec ME); `rmw_ff`, `cma_ff`, and `smb_ff5` gain country-months (+141/+120/+105 monthly) and re-rank in thin markets (changed-month correlations 0.74/1.00/0.84, revisions concentrated in small and emerging markets; US byte-identical). The ROW returns panel also no longer requires a Compustat gvkey link (US parity; no effect on current data — ex-US securities all carry gvkeys).
