...
## 02-09-2026
__Changes__:
- Fixed double-counting of CRSP CIZ delisting returns. Under the CIZ flat-file format, `MthRet`/`DlyRet` may already include the delisting payoff depending on `MthDelFlg`/`DlyDelFlg`. The pipeline now propagates `del_flag` and joins `crsp.stkdelists` on both frequencies, applying flag-conditional imputation / compounding: monthly excludes `MthDelFlg` `"A"` / `"P"` (payoff already in `MthRet`); daily excludes `DlyDelFlg = "Y"` (payoff already in `DlyRet`). Flags `"M"` (missing terminal payoff), `"G"` (gap delisting, >10 trading days), `"N"` (no delisting this period, typically a no-op because `delret` is null), and `"V"` (voluntary) all trigger compounding; missing `delret` is imputed at −0.30 for performance-related delisting reason codes. For daily `DlyDelFlg = "N"` rows that match a sedelist entry on the exact `delistingdt`, the pipeline now imputes and compounds `delret` into `ret` — previously these adjustments were monthly-only, causing the daily file to miss delisting events that the monthly file recorded. This affects `crsp_msf.parquet`, `crsp_dsf.parquet`, downstream world panels, and all rolling-window daily characteristics (rvol, beta, skew, ivol, etc., 21d–1260d). See Xia (2026, SSRN 7243220) for background.
