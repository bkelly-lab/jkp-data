...
def ecdf_r_like(x):
    ...
    unique_vals, indices, counts = np.unique(x_sorted, return_index=True, return_counts=True)
    ...
    def ecdf_func(query_points):
        ...
        ecdf_values = np.where(insert_positions > 0, cum_counts[insert_positions - 1], 0.0)
        return ecdf_values
...
def portfolios(
  ...
  ret_cutoffs_daily = None               # Data frame for daily winsorization. Neccesary when wins_ret=T and daily_pf=T
    ...
    if wins_ret:
        data = data.join(ret_cutoffs.select(["eom_lag1", "ret_exc_0_1", "ret_exc_99_9"]).rename({"eom_lag1": "eom", "ret_exc_0_1": "p001", "ret_exc_99_9": "p999"}), on='eom', how='left')
    ...
    if ind_pf:
        ...
        ind_gics = ind_gics.with_columns((pl.col("eom").dt.offset_by("1mo").dt.month_end()).alias("eom"))
        ...
        if excntry == "usa":
            ...
            ind_ff49 = ind_ff49.with_columns((pl.col("eom").dt.offset_by("1mo").dt.month_end()).alias("eom"))
...
for ex in countries:
    ...
    result = portfolios(
                  ...
                  source = settings['source'],        # Use data from "CRSP", "Compustat" or both: c("CRSP", "COMPUSTAT")
                  wins_ret = settings['wins_ret'],       # Should Compustat returns be winsorized at the 0.1% and 99.9% of CRSP returns? 
                  cmp_key = settings['cmp']['us'] if ex == 'usa' else settings['cmp']['int'],                 # Create characteristics managed size portfolios?
                  signals = settings['signals']['us'] if ex == 'usa' else settings['signals']['int'],                       # Create portfolio signals?
                  signals_standardize = settings['signals']['standardize'],           # Map chars to [-0.5, +0.5]?,
                  signals_w = settings['signals']['weight'],                  # Weighting for signals: in c("ew", "vw", "vw_cap")
...
if settings['daily_pf'] and any(sub_data and 'pf_daily' in sub_data for sub_key, sub_data in portfolio_data.items()):
    pf_daily = pl.concat([
    sub_data['pf_daily']
    for sub_key, sub_data in portfolio_data.items()
    if sub_data and 'pf_returns' in sub_data
...
# aggregating industry classification returns
    # GICS Returns
if settings['ind_pf'] and any(sub_data and 'gics_returns' in sub_data for sub_key, sub_data in portfolio_data.items()):
...
if pf_returns is not None and pf_returns.height > 0:
    hml_returns = pf_returns.group_by(["excntry", "characteristic", "eom"]).agg([
        ...
        (pl.col("signal").filter(pl.col("pf") == settings["pfs"]).first() - pl.col("signal").filter(pl.col("pf") == 1).first()).alias("signal"),
        (pl.col("n").filter(pl.col("pf") == settings["pfs"]).first() + pl.col("n").filter(pl.col("pf") == 1).first()).alias("n_stocks"),
        (pl.col("n").filter(pl.col("pf").is_in([settings["pfs"], 1])).min()).alias("n_stocks_min"),
        (pl.col("ret_ew").filter(pl.col("pf") == settings["pfs"]).first() - pl.col("ret_ew").filter(pl.col("pf") == 1).first()).alias("ret_ew"),
        (pl.col("ret_vw").filter(pl.col("pf") == settings["pfs"]).first() - pl.col("ret_vw").filter(pl.col("pf") == 1).first()).alias("ret_vw"),
        (pl.col("ret_vw_cap").filter(pl.col("pf") == settings["pfs"]).first() - pl.col("ret_vw_cap").filter(pl.col("pf") == 1).first()).alias("ret_vw_cap")
    ])
...
if lms_returns is not None:
    cluster_pfs = (lms_returns.join(cluster_labels, on="characteristic", how='left')
...
if settings['daily_pf'] and lms_daily is not None:
    cluster_pfs_daily = (lms_daily.join(cluster_labels, on="characteristic", how='left')
