...
def aug_msf_v2():
    ...
    # Month key from daily date; must match msf.yyyymm
    dt = dsf.dlycaldt.cast("date")

    # TR-only daily prices + yyyymm key
    d = (
        dsf
    ...
    # Aggregate to permno-month: highest and lowest transaction price during the month
    m = (
    ...
    # Join onto MSF, avoiding key collisions by selecting:
    #   all MSF columns + only the two new columns from m
    msf_joined = (
        msf.join(
            m,
            how="left",
    ...
    # For BA (or non-TR) monthly rows, force missing
...
def prepare_crsp_sf(freq):
    ...
    # --- expressions that require delist columns must be defined AFTER the join ---
    #   (but we can define them now since Polars expressions are lazy)
    ...
    # company-level market equity (sum ME across permnos within permco-date)
    me_company_exp = (
    ...
    # monthly volume units alignment
    if freq == "m":
