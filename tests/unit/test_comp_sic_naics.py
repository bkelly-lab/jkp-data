...
- Coalesce precedence: ``COALESCE(sica, sicb)`` prefers NA over GL when both are non-null
...
from tests.golden.comp_sic_naics_inputs import (
    build_sic_naics_inputs,
    empty_sic_naics_frame,
    sic_naics_frame,
)
...
class TestCompSicNaics:
    ...
    def test_coalesce_prefers_non_null_sic(self) -> None:
        """When NA has null SIC and GL has a real one, GL fills via COALESCE."""
        na = sic_naics_frame(
            gvkeys=["004000"],
            datadates=[date(2019, 3, 1)],
            sics=[None],
            naicses=[541110],
        )
        gl = sic_naics_frame(
    ...
    def test_coalesce_prefers_na_over_gl_when_both_non_null(self) -> None:
        """When both NA and GL have non-null SIC on the same datadate, NA wins.

        ``COALESCE(sica, sicb)`` must keep the NA value; a reversed
        ``COALESCE(sicb, sica)`` would return the higher GL SIC instead.
        """
        na = sic_naics_frame(
            gvkeys=["003000"],
            datadates=[date(2018, 5, 1)],
            sics=[6020],
            naicses=[522110],
        )
        gl = sic_naics_frame(
        ...
        row = result.row(0, named=True)
        assert row["sic"] == 6020, (
            "COALESCE(sica, sicb) must prefer NA sic over GL when both are non-null"
        )
        assert row["naics"] == 522110

    def test_hardcoded_175650_row_dropped(self) -> None:
        """The hard-coded NA filter removes ``(175650, 2005-12-31, naics=NULL)``."""
        na = sic_naics_frame(
            gvkeys=["175650", "175650"],
            datadates=[date(2005, 12, 31), date(2006, 6, 30)],
            sics=[1311, 1311],
            naicses=[None, 211120],
        )
        _write_inputs(self.paths, na, empty_sic_naics_frame())
    ...
    def test_daily_expansion_closed_left(self) -> None:
        ...
        na = sic_naics_frame(
            gvkeys=["001000", "001000"],
            datadates=[date(2020, 1, 1), date(2020, 1, 4)],
            sics=[3711, 3713],
            naicses=[336111, 336112],
        )
        _write_inputs(self.paths, na, empty_sic_naics_frame())
    ...
    def test_distinct_on_picks_lowest_sic(self) -> None:
        ...
        """
        na = sic_naics_frame(
            gvkeys=["005000", "005000"],
            datadates=[date(2020, 1, 1), date(2020, 1, 1)],
            sics=[9000, 1000],
            naicses=[999999, 111111],
        )
        _write_inputs(self.paths, na, empty_sic_naics_frame())
    ...
    def test_dedup_and_sort_invariants(self) -> None:
        ...
        na = sic_naics_frame(
            gvkeys=["002000", "001000"],
            datadates=[date(2020, 6, 15), date(2020, 1, 1)],
            sics=[2834, 7372],
            naicses=[325412, 511210],
        )
        _write_inputs(self.paths, na, empty_sic_naics_frame())
